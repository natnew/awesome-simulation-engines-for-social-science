...
* Choose the narrowest accurate section by **engine function**, not surface topic. If two sections fit, choose where readers would look first and respect cross-reference notes. Explain uncertain placement and recommend one option. Boundaries that come up most often:
  ...
  * A released end-to-end platform → Existing Systems. A method or study using LLM agents → LLM-Based Social Simulation. A way to build a population, including LLM-as-population → Synthetic Populations. Raw survey, panel, or census data → Datasets and Empirical Grounding.
  * A modelling framework → its method section (Agent-Based Modelling, MARL, LLM-Based). A cross-cutting library for inference, diagnostics, sensitivity, networks, or geospatial data → Tools and Libraries.
  * Searching interventions or mechanisms over a simulator → Policy and Intervention Modelling. Identification and estimation methods → Causal Inference.
  * Harms, fairness over time, deception, privacy leakage, and AI governance frameworks → Ethical Risk Discovery.
